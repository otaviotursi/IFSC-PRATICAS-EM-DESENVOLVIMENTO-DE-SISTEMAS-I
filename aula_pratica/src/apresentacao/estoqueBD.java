...
public class estoqueBD {
	...
	public static void InserirVenda(produtoModel produto, long qntVenda){
		try {
			Double valorTotal = produto.preco * qntVenda;
			String comando = String.format("INSERT INTO `venda`(`codProduto`, `data`, `nome`, `preco`, `quantidade`, `valorTotal`) VALUES ("+produto.codigo+",NOW(),'"+produto.nome+"','"+produto.preco+"','"+qntVenda+"', '"+valorTotal+"')");
			st.execute(comando);
	...
	public static void CriarBdProduto(){
		...
		try {
			String query = "create table if not exists estoque (codProduto integer not null AUTO_INCREMENT, nome varchar(30) not null, preco Decimal, quantidade long not null,  PRIMARY KEY(codProduto))";
			st = conn.createStatement();
			st.execute(query);
			System.out.println("BD estoque CRIADO!");
		} catch (SQLException e) {
			System.out.println("Erro ao criar bd estoque ");
			e.printStackTrace();
		}
		try {
			String query = "create table if not exists venda (id integer not null AUTO_INCREMENT, codProduto integer not null, data TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP, nome varchar(30) not null, preco Decimal not null, quantidade long not null, valorTotal long not null, PRIMARY KEY(id))";
			st = conn.createStatement();
			st.execute(query);
			System.out.println("BD estoque CRIADO!");
		} catch (SQLException e) {
			System.out.println("Erro ao criar bd venda ");
			e.printStackTrace();
		}
	}
